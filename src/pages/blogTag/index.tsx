...
import { getTagList, createTag, updateTag, deleteTag, type BlogTagVo, type BlogTagReq } from "@/api/module/blog";
import "./index.css";
...
const BlogTagPage = () => {
  ...
  const { list, loading, page, size, total, refresh, search, reset, handlePageChange } = useList<BlogTagVo>({ fetchFn: getTagList });
  ...
  const handleDelete = (record: BlogTagVo) => {
    Modal.confirm({
      title: "确认删除", content: `确定要删除标签"${record.tagName}"吗？`, okType: "danger",
      onOk: async () => { await deleteTag(record.id); message.success("删除成功"); refresh(); },
    });
  };
    ...
    <div className="page-enter">
      <div className="search-bar">
        <Form form={form} layout="inline" className="search-form">
          <Form.Item name="tagName"><Input placeholder="标签名称" allowClear /></Form.Item>
      ...
      <div className="table-card">
        <Table dataSource={list} rowKey="id" loading={loading} scroll={{ x: 700 }}
          columns={[
            ...
            { title: "操作", key: "action", fixed: "right", width: 180, render: (_: unknown, r: BlogTagVo) => (
              <Space>
      ...
      <Modal title={modalMode === "create" ? "新增标签" : modalMode === "edit" ? "编辑标签" : "查看标签"} open={modalVisible} onOk={modalMode !== "view" ? handleModalOk : undefined} onCancel={() => setModalVisible(false)} destroyOnClose>
        <Form form={form} layout="vertical" disabled={modalMode === "view"}>
          ...
          <Form.Item name="tagSlug" label="标签别名" rules={[{ required: true }]}><Input /></Form.Item>
